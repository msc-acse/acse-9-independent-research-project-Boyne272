...
Last updated on Wed Aug 28 08:36:46 2019
"""
...
class Segment_Analyser():
    ...
    def __init__(self, img, mask, clusters):
        ...
        self.labels = {str(c):c for c in np.unique(clusters)}
    ...
    def get_gsd(self, label, span=True, return_arr=False):
        ...
        # verbose
        prog_bar = progress_bar(len(segs))
...
if __name__ == '__main__':
    ...
    dummy_mask = np.vstack((mask_upper, mask_lower))

    # create a dummy clustering
    cluster = np.zeros_like(dummy_mask)
    cluster[20:, :] = 1

    # create the analysis object
    example_obj = Segment_Analyser(dummy_mask, dummy_mask, cluster)
