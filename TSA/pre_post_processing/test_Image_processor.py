...
class Test_Image_processor(unittest.TestCase):
    ...
    def test_processes(self):
        ...
        obj_color.reset(), obj_color.hsv()
        obj_color.reset(), obj_color.select_channel(channel=0)
        obj_color.reset(), obj_color.gauss(sigma=3)
        obj_color.reset(), obj_color.sobel()
        obj_color.reset(), obj_color.scharr()
        obj_color.reset(), obj_color.laplace()
        obj_color.reset(), obj_color.hog()

        
        # test each single channel function runs
        obj_grey.reset(), obj_grey.normalise()
        obj_grey.reset(), obj_grey.dilation()
        obj_grey.reset(), obj_grey.erosion()
        obj_grey.reset(), obj_grey.median()
        obj_grey.reset(), obj_grey.gabor_filters(frequency=2, n_angs=4)
        obj_grey.reset(), obj_grey.gauss_grey()
